...
const FilterSidebar = ({search,setSearch,category,setCategory,brand,setBrand,setPriceRange,allProducts, priceRange}) => {
 ...
 <input type="range" min="0" max="5000" className='w-full' value={priceRange[0]}  onChange={handleMinChange} />
  <input type="range" min="0" max="999999" className='w-full' value={priceRange[1]}  onChange={handleMaxChange} />
    </div>
    {/*Reset button */}
    <Button  onClick={resetFilters} className= 'bg-blue-600 text-white mt-5 cursor-pointer w-full'>Reset Filters</Button>
    </div>
  )
}
